...
- Phase 4 — `cd backend && npm test -- src/modules/remote-imports/resource-control.test.ts src/modules/remote-imports/hls/segments.test.ts src/modules/remote-imports/hls/ffmpeg.test.ts` → 3 files / 35 tests passed (global aggregate permit cap, queued cancellation, HLS behavior, and resource diagnostics); `cd backend && npm test -- src/modules/remote-imports src/modules/s3/s3.service.test.ts` → 32 files / 399 tests passed; `cd backend && npm test` → 96 files / 1,108 tests passed; `cd backend && npm run build` → passed (2026-09-11).
...
- Phase 4: FIFO global segment and FFmpeg permits cap aggregate activity; cancellation removes queued waiters and all acquire paths release permits in `finally`. Their active/waiting counters are included in capacity-deferral diagnostics.
